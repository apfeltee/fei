...
int fei_dbgutil_printconstir(FeiState* state, const char* name, FeiBytecodeList* chunk, int offset)
{
    ...
    fei_tostring_value(state, state->iowriter_stderr, fei_valarray_get(chunk->constants, constant), true);
    fprintf(stderr, "'\n");
        ...
        case OP_CLOSURE:
            {
                ...
                fei_tostring_value(state, state->iowriter_stderr, fei_valarray_get(chunk->constants, constant), true);// accessing the value using the index
                fprintf(stderr, "\n");
